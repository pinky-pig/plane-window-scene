...
5. 设置视角移动。这里是有一个移动的线的点集，添加一个模型跟着线运动，然后将相机视角跟模型绑定，这样相机就跟着线运动了。但是遇到了一个小坑，就是模型设置隐藏的时候 （ show:false ） ，模型并不会运动了，相机视角也不会运动了，所以这里使用了一个小技巧，就是将模型的 scale 设置为 0 ，这样模型就看不见了，但是还是会运动的。
